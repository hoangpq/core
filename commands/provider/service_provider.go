...
type ServiceProvider struct {
	client grpcclient.CoreClient
}

// NewServiceProvider creates new ServiceProvider.
func NewServiceProvider(c grpcclient.CoreClient) *ServiceProvider {
	return &ServiceProvider{client: c}
}

// ServiceByID finds service based on given id.
func (p *ServiceProvider) ServiceByID(id string) (*grpcclient.Service, error) {
	serviceReply, err := p.client.GetService(context.Background(), &grpcclient.GetServiceRequest{ServiceID: id})
	if err != nil {
		return nil, err
	}
...
func (p *ServiceProvider) ServiceDeleteAll() error {
	rep, err := p.client.ListServices(context.Background(), &grpcclient.ListServicesRequest{})
	if err != nil {
		return err
	}
	...
	for _, s := range rep.Services {
		_, err := p.client.DeleteService(context.Background(), &grpcclient.DeleteServiceRequest{ServiceID: s.ID})
		if err != nil {
...
func (p *ServiceProvider) ServiceDelete(ids ...string) error {
	var errs xerrors.Errors
	for _, id := range ids {
		_, err := p.client.DeleteService(context.Background(), &grpcclient.DeleteServiceRequest{ServiceID: id})
		if err != nil {
			errs = append(errs, err)
		}
	}
...
func (p *ServiceProvider) ServiceListenEvents(id, eventFilter string) (chan *grpcclient.EventData, chan error, error) {
	stream, err := p.client.ListenEvent(context.Background(), &grpcclient.ListenEventRequest{
		ServiceID:   id,
	...
	resultC := make(chan *grpcclient.EventData)
...
func (p *ServiceProvider) ServiceListenResults(id, taskFilter, outputFilter string, tagFilters []string) (chan *grpcclient.ResultData, chan error, error) {
	stream, err := p.client.ListenResult(context.Background(), &grpcclient.ListenResultRequest{
		ServiceID:    id,
	...
	resultC := make(chan *grpcclient.ResultData)
	errC := make(chan error)
...
func (p *ServiceProvider) ServiceLogs(id string, dependencies ...string) (logs []*Log, close func(), err error) {
	if len(dependencies) == 0 {
		resp, err := p.client.GetService(context.Background(), &grpcclient.GetServiceRequest{
			ServiceID: id,
		})
		if err != nil {
			return nil, nil, err
	...
	stream, err := p.client.ServiceLogs(ctx, &grpcclient.ServiceLogsRequest{
		ServiceID:    id,
...
func (p *ServiceProvider) listenServiceLogs(stream grpcclient.Core_ServiceLogsClient, logs []*Log,
	errC chan error) {
	for {
		...
		for _, log := range logs {
			if log.Dependency == data.Dependency {
				...
				switch data.Type {
				case grpcclient.LogData_Standard:
					out = log.Standard
				case grpcclient.LogData_Error:
					out = log.Error
				}
...
func (p *ServiceProvider) ServiceExecuteTask(id, taskKey, inputData string, tags []string) error {
	_, err := p.client.ExecuteTask(context.Background(), &grpcclient.ExecuteTaskRequest{
		ServiceID:     id,
		TaskKey:       taskKey,
		InputData:     inputData,
		ExecutionTags: tags,
...
func (p *ServiceProvider) ServiceStart(id string) error {
	_, err := p.client.StartService(context.Background(), &grpcclient.StartServiceRequest{ServiceID: id})
	return err
}

// ServiceStop stops a service.
func (p *ServiceProvider) ServiceStop(id string) error {
	_, err := p.client.StopService(context.Background(), &grpcclient.StopServiceRequest{ServiceID: id})
	return err
}
...
func (p *ServiceProvider) ServiceList() ([]*grpcclient.Service, error) {
	reply, err := p.client.ListServices(context.Background(), &grpcclient.ListServicesRequest{})
	if err != nil {
		return nil, err
	}
