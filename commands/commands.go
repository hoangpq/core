...
type ServiceExecutor interface {
	ServiceByID(id string) (*grpcclient.Service, error)
	ServiceDeleteAll() error
	ServiceDelete(ids ...string) error
	ServiceDeploy(path string) (id string, valid bool, err error)
	ServiceListenEvents(id, eventFilter string) (chan *grpcclient.EventData, chan error, error)
	ServiceListenResults(id, taskFilter, outputFilter string, tagFilters []string) (chan *grpcclient.ResultData, chan error, error)
	...
	ServiceList() ([]*grpcclient.Service, error)
