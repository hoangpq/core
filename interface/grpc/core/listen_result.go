...
func (s *Server) ListenResult(request *grpcclient.ListenResultRequest, stream grpcclient.Core_ListenResultServer) error {
	...
	for {
		...
		case execution := <-ln.Executions:
			...
			if err := stream.Send(&grpcclient.ResultData{
