...
func listenEvents(wf *Workflow) (err error) {
	...
	stream, err := wf.client.ListenEvent(context.Background(), &grpcclient.ListenEventRequest{
...
func (wf *Workflow) validEvent(data *grpcclient.EventData) bool {
...
func listenResults(wf *Workflow) (err error) {
	...
	stream, err := wf.client.ListenResult(context.Background(), &grpcclient.ListenResultRequest{
	...
	for {
		var data *grpcclient.ResultData
		data, err = stream.Recv()
		if err != nil {
			break
		}
...
func (wf *Workflow) validResult(data *grpcclient.ResultData) bool {
